...
const fmt = iso => { if(!iso) return ''; const d=new Date(iso); if(isNaN(d)) return iso; const z=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}/${z(d.getMonth()+1)}/${z(d.getDate())} ${z(d.getHours())}:${z(d.getMinutes())}`; };
...
async function loadData(){
  ...
  qs('#menuFilter').innerHTML = `<option value="">メニュー：すべて</option>` + menus.map(m=>`<option value="${esc(m)}">${esc(m)}</option>`).join('');
...
function applyFilter(){
  ...
  let arr = state.customers.filter(c => !q || [c.name,c.email,c.phone].some(v => (v||'').toLowerCase().includes(q)));
...
function applySort(){
  const [key,dir] = qs('#sort').value.split(':'); state.sortKey=key; state.sortDir=dir;
  const m = dir==='asc' ? 1 : -1;
...
function attach(){
  ['#q','#from','#to','#menuFilter','#tagFilter'].forEach(sel=>{
    qs(sel).addEventListener('input', applyFilter); qs(sel).addEventListener('change', applyFilter);
  });
  ...
  document.querySelectorAll('input[name="view"]').forEach(r=>{
    r.addEventListener('change', ()=>{ document.body.className = 'view-'+r.value; });
  });
}

function exportCsv(){
  const headers = ['氏名','氏名（カナ）','性別','電話','メール','住所','生年月日','初回予約日','最終予約日','回数','直近メニュー','担当者','タグ','メモ'];
